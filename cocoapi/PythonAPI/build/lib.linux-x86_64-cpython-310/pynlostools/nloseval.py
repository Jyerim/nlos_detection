...
class NLOSeval:
    # Interface for evaluating detection on the Microsoft NLOS dataset.
    #
    # The usage for CocoEval is as follows:
    #  nlosGt=..., nlosDt=...       # load dataset and results
    #  E = CocoEval(nlosGt,nlosDt); # initialize CocoEval object
    ...
    #
    # See also nlos, mask, pynlosDemo, pynlosEvalDemo
    #
    # Microsoft NLOS Toolbox.      version 2.0
    # Data, paper, and tutorials available at:  http://msnlos.org/
    # Code written by Piotr Dollar and Tsung-Yi Lin, 2015.
    # Licensed under the Simplified BSD License [see nlos/license.txt]
    def __init__(self, nlosGt=None, nlosDt=None, iouType='segm'):
        '''
        Initialize CocoEval using nlos APIs for gt and dt
        :param nlosGt: nlos object with ground truth annotations
        :param nlosDt: nlos object with detection results
        :return: None
        '''
        if not iouType:
            print('iouType not specified. use default iouType segm')
        self.nlosGt   = nlosGt              # ground truth NLOS API
        self.nlosDt   = nlosDt              # detections NLOS API
    ...
    def _prepare(self):
        ...
        :return: None
        '''
        def _toMask(anns, nlos):
            # modify ann['segmentation'] by reference
            for ann in anns:
                rle = nlos.annToRLE(ann)
        ...
        if p.iouType == 'segm':
            _toMask(gts, self.nlosGt)
            _toMask(dts, self.nlosDt)
        # set ignore flag
        for gt in gts:
    ...
    def summarize(self):
        ...
        '''
        def _summarize( ap=1, iouThr=[0.5,0.95], areaRng='all', maxDets=10 ):
            p = self.params
            ...
            iouStr = '{:0.2f}:{:0.2f}'.format(iouThr[0],iouThr[-1]) \
                if isinstance(iouThr,list)  else '{:0.2f}'.format(iouThr)
            ...
            if ap == 1:
                ...
                # IoU
                if isinstance(iouThr,list):
                    t = np.where(np.isclose(iouThr[0],p.iouThrs))[0]
                    t2 = np.where(np.isclose(iouThr[-1],p.iouThrs))[0]
                    s = s[int(t):int(t2)]
                else:
        ...
        def _summarizeDets():
            stats = np.zeros((13,))
            stats[0] = _summarize(1, iouThr=[0.5,0.95])
            stats[1] = _summarize(1, iouThr=.1, maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.maxDets = [1,5,10]
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 96 ** 2], [96 ** 2, 128 ** 2], [128 ** 2, 1e5 ** 2]]
