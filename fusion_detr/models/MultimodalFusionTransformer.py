...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000):
        ...
        self.conv1_rf = nn.Conv2d(4, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1_rf = nn.BatchNorm2d(64)

        self.layer1_rf = copy.deepcopy(self.layer1_laser)
        self.layer2_rf = copy.deepcopy(self.layer2_laser)
        self.layer3_rf = copy.deepcopy(self.layer3_laser)
        self.layer4_rf = copy.deepcopy(self.layer4_laser)
        self.linear_rf = nn.Linear(8192, 4096)
...
def MultimodalFusionTransformer():
    return ResNet(BasicBlock, [2, 2, 2, 2])

if __name__ == "__main__":
    device = 'cuda:0'
    net = MultimodalFusionTransformer().cuda()
    B = 2
