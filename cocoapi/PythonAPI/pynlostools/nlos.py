...
class NLOS:
    def __init__(self, annotation_file=None, mode='Val'):
        """
        ...
        if not annotation_file == None:
            ...
            if '2021' in annotation_file:
                if mode == 'Train':
                    self.bad_group_list = [789, 837, 838, 839]
            elif '2022' in annotation_file:
                if mode == 'Train':
                    self.bad_group_list = [2476,205] 
                elif mode == 'Val':
                    self.bad_group_list = [696,108]
            self.createIndex(self.bad_group_list)

    def createIndex(self, bad_group_list):
        # create index
        ...
        if 'annotations' in self.dataset:
            for ann in self.dataset['annotations']:
                if ann['image_group_id'] in bad_group_list:
                    continue
        ...
        if 'image_groups' in self.dataset:
            for img_group in self.dataset['image_groups']:
                if img_group['id'] in bad_group_list:
                    continue
    ...
    def loadRes(self, resFile):
        ...
        assert set(annsImgIds) == (set(annsImgIds) & set(self.getImgIds())), \
               'Results do not correspond to current nlos set{}'.format(annsImgIds)
        ...
        res.createIndex(self.bad_group_list)
        return res
